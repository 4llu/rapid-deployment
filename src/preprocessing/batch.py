...
def FFT_w_phase(support_query_set, config, device):
    ...
    support_query_set = torch.cat(
        [torch.abs(support_query_set), torch.angle(support_query_set)], dim=-2
    )
...
def random_freq_masking(support_query_set, config, device):
    ...
    for i in range(num_bands):
        support_query_set[
            :, :, :, band_locations[i] : band_locations[i] + band_widths[i]
        ] = 0
...
def combined_freq_masking(support_query_set, config, device):
    ...
    assert (
        len(config["combined_freq_masking_probabilities"]) == 4
    ), "4 probabilities are required!"
...
def preprocess_batch(support_query_set, split, config, device):
    ...
    if "add_white_noise" in config["preprocessing_batch"]:
        # * Only use augmentations for training
        if split == "train":
    ...
    if "low_freq_masking" in config["preprocessing_batch"]:
        # ? Should this be in use for evaluation and testing?
        # * Only use augmentations for training
        if split == "train":
