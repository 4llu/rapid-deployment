...
def order_tracking(class_support_query_set, config, idx):
    # FIXME Not checked for other datasets than ARotor
    new_class_support_query_set = []
    ...
    len_shortest_fft = config["sample_cut"] + (0 if config["include_FFT_DC"] else 1)
    if config["sample_cut"] == -1:
        len_shortest_fft = len(
            np.fft.rfft(np.ones(rpm_rotation_lengths[idx[1]]))
        )  # FIXME No need to calculate every time
    new_support_query_set = np.fft.rfft(new_support_query_set)[
        # :,
        # :,
        # :,
        # 1:len_shortest_fft
        ...
        :,
        :,
        0 if config["include_FFT_DC"] else 1 : len_shortest_fft,
    ]  # Drop DC
...
def preprocess_class_batch(class_support_query_set, split, config, idx, query_samples):
    ...
    if "FFT" in config["preprocessing_class_batch"]:
        raise Exception(
            "SLOWER THAN `batch`-LEVEL IMPLEMENTATION! ARE YOU SURE YOU WANT TO USE THIS? IN THAT CASE COMMENT THIS `raise`"
        )
